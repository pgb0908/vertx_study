 ...
 */
final class VertxWriteStreamSubscriber implements Flow.Subscriber<byte[]> {

    private final WriteStream<Buffer> target;
    private final CompletableFuture<Void> done;
    private Flow.Subscription subscription;

    VertxWriteStreamSubscriber(WriteStream<Buffer> target, CompletableFuture<Void> done) {
        this.target = target;
        this.done = done;
    }
